			...
			<when test="@rdf:resource|@rdf:about">
				<variable name="mediatype">
					<choose>
						...
						<when test="self::*[substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/parent::*[self::rdf:Bag|self::rdf:Seq]/dc:format">
							<for-each select="..">
								<call-template name="mediatype"/>
							</for-each>
						</when>
				...
				<choose>
					...
					<when test="starts-with($mediatype, 'image/')">
						<html:img alt="{@dc:abstract}" data-src="{@rdf:about|@rdf:resource}"/>
					</when>
...
window.addEventListener(`load`, ( ) => {
	if ( location.search.length > 1 ) {
		const e = document.getElementById(decodeURIComponent(location.search.substring(1)))
		if ( !(e == null || !e.hasAttribute("data-srcdoc")) )
			try {
				const s = (new DOMParser).parseFromString(e.getAttribute("data-srcdoc"), "application/xhtml+xml")
				document.head.replaceWith(document.importNode(s.head, true))
				document.body.replaceWith(document.importNode(s.body, true))
				for ( const m of document.body.querySelectorAll `iframe[data-src],audio[data-src],video[data-src],img[data-src]` ) {
					const n = m.cloneNode()
					n.src = m.dataset.src
	...
	<template match="html:a">
		...
		<variable name="target" select="document($rdf)//bns:includes/*[@rdf:about=current()/@href]|document($rdf)//bns:hasProjects/*/*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/*[@rdf:about=current()/@href]"/>
	...
	<template name="formatted">
		<choose>
			<when test="self::bns:Project|parent::*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/../parent::bns:hasProjects">
				<variable name="index" select="number(substring(local-name(..), 2))"/>
			...
			<when test="not(bns:index)">
				<html:code>
					<choose>
						...
						<when test="../../owl:sameAs[starts-with(current()/@rdf:about, ../../@rdf:resource)]">
							<value-of select="substring-after(@rdf:about, ../../owl:sameAs[starts-with(current()/@rdf:about, @rdf:resource)][1]/@rdf:resource)"/>
						</when>
						<otherwise>
							<call-template name="shorten">
								<with-param name="uri" select="@rdf:about"/>
	...
	<template name="navigate">
		<variable name="siblings">
			<choose>
				<when test="parent::*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/../parent::bns:hasProjects">
					<for-each select="../../../../bns:hasProjects/*/*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/*">
						<sort select="substring(local-name(..), 2)" data-type="number"/>
						<html:span>
		...
		<html:nav>
			...
			<choose>
				<when test="parent::*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/../parent::bns:hasProjects/..">
					<html:a data-nav="parent">
						<attribute name="href">
							<text>#</text>
							<for-each select="parent::*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/../parent::bns:hasProjects/..">
								<call-template name="shorten">
									<with-param name="uri" select="@rdf:about"/>
								</call-template>
							</for-each>
				...
				<when test="parent::bns:includes/..|parent::bns:hasNote/..">
					<html:a data-nav="parent">
						<attribute name="href">
							<text>#</text>
							<for-each select="../..">
			...
			<choose>
				<when test="bns:hasProjects/*/*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/*">
					<html:a data-nav="child">
						<attribute name="href">
							<text>#</text>
							<for-each select="bns:hasProjects[1]/*[1]/*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#'][1]/*[1]">
								<call-template name="shorten">
									<with-param name="uri" select="@rdf:about"/>
								</call-template>
							</for-each>
				...
				<when test="bns:includes/*">
					<html:a data-nav="child">
						<attribute name="href">
							<text>#</text>
							<for-each select="bns:includes[1]/*[1]">
								<call-template name="shorten">
									<with-param name="uri" select="@rdf:about"/>
								</call-template>
							</for-each>
	...
	<template match="*[parent::bns:includes|parent::*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/../parent::bns:hasProjects|parent::bns:hasNote]">
		<variable name="contents">
	...
	<template match="*[parent::bns:includes|parent::*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/../parent::bns:hasProjects|parent::bns:hasNote]" mode="header">
		<html:header>
			<html:p>
				<for-each select="ancestor::*[parent::bns:includes|parent::*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/../parent::bns:hasProjects|parent::bns:hasNote]">
					<html:a>
						<attribute name="href">
							<text>#</text>
							<call-template name="shorten">
	...
	<template match="*[parent::bns:includes|parent::*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/../parent::bns:hasProjects|parent::bns:hasNote]" mode="list">
		<html:li>
			<choose>
				<when test="parent::*[string-length(local-name())>1][substring(local-name(), 1, 1)='_'][translate(substring(local-name(), 2, 1), '123456789', '')=''][translate(substring(local-name(), 3), '0123456789', '')=''][namespace-uri()='http://www.w3.org/1999/02/22-rdf-syntax-ns#']/../parent::bns:hasProjects">
					<attribute name="value">
						<value-of select="substring(local-name(), 2)"/>
					</attribute>
				</when>
				<when test="parent::bns:includes">
					<attribute name="value">
